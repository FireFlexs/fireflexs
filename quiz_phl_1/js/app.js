(() => {
  ...
  const state = {
    ...
    examTimeLimit: 3600 // 60 minutes in seconds
  };
  ...
  function renderHome() {
    el.app.innerHTML = `
      <section class="hero card">
        <div class="hero__top">
          <div>
            <p class="eyebrow">Тест-тренажёр</p>
        ...
        <div class="exam-section">
          <div class="exam-divider">
            <span>или</span>
          </div>
          <button class="exam-btn" id="examBtn" type="button">
            <span class="exam-btn__icon">🎓</span>
            <span class="exam-btn__content">
              <span class="exam-btn__title">Режим экзамена</span>
              <span class="exam-btn__meta">25 вопросов · 60 минут · все темы</span>
            </span>
          </button>
        </div>
  ...
  function renderQuestion() {
    ...
    const timerHtml = state.examMode ? `
      <div class="exam-timer ${getTimerClass()}" id="examTimer">
        <span class="exam-timer__dot"></span>
        <span class="exam-timer__value" id="examTimerVal">${formatTime(Math.max(0, state.examTimeLimit - state.examTimerSec))}</span>
      </div>
    ` : '';
    ...
    el.app.innerHTML = `
      <section class="card">
        <div class="quiz-top">
          <div>
            <p class="eyebrow">${state.examMode ? '🎓 Экзамен · ' : ''}Вопрос ${state.index + 1} из ${total}</p>
  ...
  function getTimerClass() {
    const remaining = state.examTimeLimit - state.examTimerSec;
  ...
  function finishQuiz() {
    ...
    const timeUsed = state.examMode ? formatTime(state.examTimerSec) : null;
    const timeExpired = state.examMode && state.examTimerSec >= state.examTimeLimit;
    ...
    el.app.innerHTML = `
      <section class="card result-card">
        <p class="eyebrow">${state.examMode ? '🎓 Экзамен · ' : ''}Результат</p>
        <h2>${correct} из ${total}</h2>
        <p class="result-percentage">${percent}%${timeExpired ? ' · Время вышло!' : ''}</p>
        ...
        <div class="stats-grid stats-grid--result">
          ...
          ${timeUsed ? `
          <div class="stat">
            <span class="stat__value">${timeUsed}</span>
  ...
  function resetState() {
    ...
    state.examTimerSec = 0;
  }

  function restartQuiz() {
    const wasExam = state.examMode;
    state.started = false;
    state.quiz = [];
    state.index = 0;
    state.score = 0;
  ...
  function startExamTimer() {
    ...
    state.examTimerInterval = setInterval(() => {
      state.examTimerSec++;
      const timerEl = $('examTimerVal');
      const timerWrap = $('examTimer');
      if (timerEl) {
        const remaining = Math.max(0, state.examTimeLimit - state.examTimerSec);
        timerEl.textContent = formatTime(remaining);
      }
      if (timerWrap) {
        timerWrap.className = 'exam-timer ' + getTimerClass();
      }
      // Auto-finish when time runs out
      if (state.examTimerSec >= state.examTimeLimit) {
        finishQuiz();
      }
    }, 1000);
  }
  ...
  async function startExam() {
    state.examMode = true;
    state.mode = 'all';
    renderLoading('Подготовка экзамена…');

    const pool = await loadMode('all');
    state.pool = pool;

    const count = Math.min(25, pool.length);
